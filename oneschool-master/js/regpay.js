document
  ...
  .addEventListener("submit", function (event) {
    ...
    const referenceId =
      "REF" +
      Math.floor(Math.random() * 1000000) + generateRandomString(3) + 
      Math.floor(Math.random() * 1000000) + generateRandomString (5);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (dateInput) {
    // Set a placeholder using a data attribute or directly
    ...
    dateInput.addEventListener("focus", function () {
      dateInput.classList.remove("placeholder-shown");
    });

    // Add the placeholder back if the input is empty on blur
